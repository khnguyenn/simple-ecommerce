...
def items(request):
    ...
    # Query for items
    if query:
        items = items.filter(Q(description__icontains=query) | Q(description__icontains=query))
...
def detail(request, pk):
    ...
    related_items = Items.objects.filter(category=item.category,is_sold=False).exclude(pk=pk)[:4]
